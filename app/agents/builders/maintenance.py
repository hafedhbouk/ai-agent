...
logger = get_logger("agents.builders.maintenance")


class MaintenanceAgent(GenericAgent):
    ...
    async def run(self, message: str, context: Optional[AgentContext] = None) -> AgentResponse:
        logger.info(f"Running maintenance agent with message: {message[:100]}")
        return await super().run(message, context)

    async def stream(self, message: str, context: Optional[AgentContext] = None):
        logger.info(f"Streaming maintenance agent with message: {message[:100]}")
